...
def provider_keys(
    ...
    **بلا أيّ قيمة مفتاح** (FR-013): الكاتب لا يكتب إلّا أعداداً ومؤشّرات، وهذه
    ...
    صفٌّ لكل (مالك، مزوّد) لا صفٌّ لكل مزوّد: المزوّدُ الواحد قد يوجد في
    `FomoChain` و`FomoEVMReplay` معاً بحالتين مستقلّتين (عمليّتان، ذاكرتان)،
    ودمجُهما يخفي نفادَ رصيدٍ في إحداهما تحت سلامة الأخرى. (كان حوضُ GoldRush
    هو هذه الحالة قبل حذف المزوّد، ومسارُ الإعادة اليوم بلا مفتاح أصلاً.)
    ...
    for key, raw in all_meta(conn).items():
        ...
        for provider, pool in sorted((pools or {}).items()):
            ...
            out.append({
                ...
                # مزوّدٌ مُسكَت لبقيّة عمر العمليّة (نفادُ رصيدٍ ⇒ 402، كما حدث
                # لـGoldRush): الأحواض تبدو سليمة والمزوّد معطَّل، فيُقال صراحةً.
