...
**Input**: Design documents from `/specs/001-fomo-family-api/` â€” plan.md,
...
- **[P]**: Can run in parallel (different files, no dependencies on
  incomplete tasks)
- **[Story]**: User story this task belongs to (US1, US2, US3) â€” required
...
Python 3.12 آ· FastAPI آ· httpx آ· Uvicorn آ· Pydantic v2 آ· Playwright (Privy
login) آ· Redis (volatile sessions, rate limits, pub/sub) آ· pytest +
...
- [X] T009 [P] Implement consumer API key issue/verify + volatile session
  store (Redis, short TTL, no disk persistence â€” FR-013) in
...
- [X] T011 Implement fomo upstream client adapter base (httpx AsyncClient,
  ...
  UPSTREAM_CHANGED shape-change detection, never-fabricate policy â€” FR-007) in
...
**Checkpoint**: Foundation ready â€” envelope, errors, rate limiting, auth
...
**Independent Test**: Query the leaderboard â†’ ranked traders; resolve a
chosen trader's profile by id â†’ public stats. No other story required.
...
- [X] T016 [P] [US1] Contract test for GET /v1/traders/{trader_id} (profile +
  metrics, null for missing fomo fields â€” FR-007) with respx-mocked upstream
...
- [X] T020 [US1] Implement FomoClient.get_leaderboard + get_trader_profile
  methods (map fomo internal endpoints â†’ Trader/Leaderboard shapes) in
  api/src/fomo_api/clients/fomo_client.py
- [X] T021 [US1] Implement leaderboard service (pagination, freshness,
  upstream-unavailable â†’ 502/503) in api/src/fomo_api/services/leaderboard_service.py
...
- [X] T025 [US1] Implement POST /v1/auth/login + /v1/auth/logout routes
  (guided Privy login â†’ consumer key; revoke drops session â€” FR-013) in
  api/src/fomo_api/api/auth.py

**Checkpoint**: User Story 1 fully functional and testable independently.
MVP delivered â€” leaderboard + profiles + auth reachable via API with no
...
**Independent Test**: Given a trader id, request the activity feed â†’
...
- [X] T026 [P] [US2] Contract test for GET /v1/traders/{trader_id}/activity
  ...
  inactive trader â€” US2 acceptance 3, 422 on bad from/to range) with
...
- [X] T028 [P] [US2] Create Token Pydantic model (price/volume/liquidity/
  market_cap all nullable â€” FR-007) in api/src/fomo_api/models/token.py
...
alert on a qualifying buy; unsubscribe â†’ no further alerts. Functions for a
...
- [X] T033 [P] [US3] Contract test for GET /v1/alerts/stream SSE (alert
  event on qualifying buy, no spurious alerts â€” US3 acceptance 2, events
  stop after unsubscribe â€” acceptance 3) in
...
- [X] T043 [P] Integration test for graceful degradation (upstream 5xx â†’
  502 UPSTREAM_UNAVAILABLE / 503 STALE_DATA, never fabricated 200 â€” FR-007) in
  api/tests/integration/test_degradation.py
- [X] T044 [P] Integration test for rate limiting (429 RATE_LIMITED on
  per-consumer breach â€” FR-008) in api/tests/integration/test_rate_limit.py
- [X] T045 [P] Additional unit tests for pagination bounds (page_size 1â€“100),
...
- [X] T046 [P] Structured logging + UPSTREAM_CHANGED detection audit across
  services (clear non-misleading errors â€” FR-009) in api/src/fomo_api/
...
- [X] T048 Run quickstart.md validation (Scenarios 1â€“4) end-to-end against a
...
- **User Story 1 (Phase 3)**: Depends on Phase 2. MVP â€” deliver and validate
  first.
...
- All Foundational tasks marked [P] can run in parallel within Phase 2
  (T006, T007, T008, T009, T012, T013) â€” T010, T011, T014 are sequential
  anchors.
...
- Within US3: T034/T035 (model + pubsub) parallel; T032/T033 contract tests
  parallel.
- All Polish tasks marked [P] can run in parallel (T041â€“T047).
...
3. Complete Phase 3: User Story 1 (write tests first â†’ implement â†’ green)
...
2. Add US1 â†’ validate Scenario 1 â†’ Deploy/Demo (MVP!)
3. Add US2 â†’ validate Scenario 2 â†’ Deploy/Demo
4. Add US3 â†’ validate Scenario 3 â†’ Deploy/Demo
5. Polish (Phase 6) â†’ validate Scenario 4 (degradation) â†’ release
