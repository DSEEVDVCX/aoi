...
- **اللوحة تفتح القاعدة `mode=ro`** — لا كتابة إلى SQLite ولا تعطيل للمسجّل.
  الكتابة الوحيدة هي أوامر المستخدم الصريحة في بطاقة مفاتيح Helius إلى المخزن
  المحلي المحدد بـ`AOI_HELIUS_KEYS_PATH`، تحت قفل وكتابة ذرية.
...
- يمكن ضبط `AOI_HELIUS_KEYS_PATH` على مخزن `crib` المحلي؛ يقرأ المفاتيح المفعّلة
  ويبدأ منها دائرياً، ثم يحوّل عبر كل المفاتيح الحيّة عند 429 أو overload أو
  نفاد credits، من دون نسخها إلى المستودع أو أثر RPC. نفاد الرصيد الصريح يعطّل
  المفتاح في المخزن حتى يعاد تفعيله بعد التجديد؛ 429 المتكرر يستبعده للجلسة فقط.
- لوحة المفاتيح لا تعيد القيمة الكاملة للمتصفح بعد الحفظ؛ تعرض أول/آخر أربعة
  محارف فقط. كل إضافة/حذف/تعطيل محمي برمز CSRF وحارس Host محلي.
