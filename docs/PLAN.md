...
**الوضع الحالي** (محدَّث 2026-08-29): الجمع والتوسيم وبناء الصفوف تعمل
تلقائياً. مستخرج الميزات الحالي **fv16 = 171 ميزة**. بوابة الضابطة v3:
**426/500** ناضجة؛ الفحص الأولي مفتوح، وقرار الاعتماد ما يزال مغلقاً (74 متبقية،
ETA تقريبي نحو 5 أيام بالمعدل الحالي). هذه الأرقام تتغير حيّاً؛ المرجع الملزم
للعدد والطزاجة هو صفحة «التوسيم والنتائج» والمسار `/api/labeling`، ولصحة المصادر
هو تقرير `recorder/data_readiness.py` المولّد — لا الأرقام التاريخية أدناه.

**قرار البيانات:** التدريب على الحيّ فقط — الصفوف الرجعية تفتقر بنيوياً إلى العائلات اللحظية
...
الحيّة** (محفوظ في نسخة `.bak`) ويُستبعَد بـ`is_live=1`. **بوّابة النضج
التاريخية للضابطة القديمة اكتملت (2026-08-02): كل الشروط وقتها مستوفاة،
والضابطة القديمة 123/100؛ لا تُنقل أهليتها إلى v3.** المرحلة 1 شُغّلت في
...
| ضابطة `v3` ناضجة للفحص الأولي | ≥ 100 | ✅ **426/100** في 2026-08-29؛ فحص أولي متاح، وليست بوابة اعتماد |
| ضابطة `v3` ناضجة للقرار الأساسي | **≥ 500** | ⏳ **426/500**؛ العتبة الملزمة قبل قبول/رفض الأطروحة |
| نسبة `status='ok'` | ≥ 85% | ✅ ~99% |
| أيام تقويمية حيّة | ≥ 5 | ✅ **7.7** (2026-07-25 – 08-02) |
| **مستخرج الميزات** | مبنيّ ومختبَر | ✅ **fv16** (171 ميزة · تقرير الجاهزية: model=ok) |
...
```
0. بوّابة النضج        ⏳ الإشارات مكتملة؛ الضابطة 426/500
2. بناء المجموعة        ✅ مبنيّ ومشغَّل: training_rows · fv16 · 171 ميزة
1. الإشارة ضدّ الضابطة  ⏳ جمع v3؛ الفحص الأولي مفتوح، قرار الاعتماد عند 500
