...
FAM["trigger"] = [
    ...
    "top_traders_listed", "top_trader_match_ratio", "buyers_best_rank",
    "rank_le_10", "rank_le_50", "ticker_len", "ticker_has_digit",
    "ticker_non_ascii", "hour_utc", "dow",
]
FAM["toptrader_periods"] = [
...
FAM["token_static"] = [
    ...
    "is_scam", "mintable", "freezable", "socials_count", "has_twitter",
    "creator_prior_tokens", "name_len", "name_non_ascii", "decimals",
    "exchanges_count", "listed_on_exchange", "has_cmc_id",
...
FAM["social_thesis"] = [
    ...
    "thesis_history_days", "social_thesis_total", "social_thesis_authors",
    "social_holder_authors", "social_holder_ratio", "social_replies",
    "social_snapshot_age_min", "social_total_delta_1h",
    "social_total_growth_1h", "social_authors_delta_1h",
]
FAM["price_path"] = [
    "ret_1h_before", "ret_4h_before", "ret_24h_before", "ret_7d_before",
    "vol_24h_before", "flat_ratio_24h", "up_candle_ratio_24h",
    "dist_from_ath", "ath_history_complete", "ath_history_days",
    "bars_history_h", "bars_count_24h", "bar_vol_1h", "bar_vol_24h",
    "vol_surge_1h",
]
FAM["market_snapshot"] = [
    "liquidity", "holders", "top10_holders_pct", "volume_24h",
    "buy_count_24h", "sell_count_24h", "buy_sell_ratio_24h",
    "unique_buys_24h", "unique_sells_24h", "tick_age_min",
    "tick_change_1h", "tick_change_4h", "tick_change_24h",
    "tick_volume_1h", "tick_volume_4h", "tick_txn_1h", "tick_txn_24h",
    "volume_to_liquidity", "liquidity_to_mcap", "float_ratio",
    "tick_rich_age_min",
]
FAM["chain_ownership"] = [
    "chain_top10_pct", "chain_holder_count", "chain_holders_delta_1h",
    "chain_holders_growth_1h", "chain_holders_span_min", "holders_age_min",
...
FAM["onchain_contract_evm"] = [
    "onchain_code_size", "onchain_function_count", "onchain_is_proxy",
    "onchain_owner_renounced", "onchain_has_mint_fn",
    "onchain_has_pause_fn", "onchain_has_blacklist_fn",
    "onchain_has_fee_setter", "onchain_has_limit_setter",
    "onchain_has_trading_switch", "onchain_contract_age_min",
]
FAM["flow"] = [c for c in cols if c.startswith("flow_")]
FAM["regime"] = ["sol_ret_4h", "sol_ret_24h", "eth_ret_24h"]
FAM["density"] = [
...
META = ["kind", "key", "token_address", "network_id", "entry_ts",
        "asset_class", "split", "is_independent", "is_live", "status",
        "suspect_bars", "feature_version", "built_at"]

covered = set(META)
for v in FAM.values():
    covered |= set(v)
print("columns not assigned to any family:", sorted(set(cols) - covered))
dup = [c for c, n in Counter(c for v in FAM.values() for c in v).items() if n > 1]
print("columns in >1 family:", dup)
print("family sizes:", {k: len(v) for k, v in FAM.items()})

with open(os.path.join(HERE, "probe_ef_fam.pkl"), "wb") as fh:
    pickle.dump(FAM, fh)
