...
async def test_cycle_runs_live_backfill_before_historical_replay(monkeypatch):
    ...
    async def fake_assist(_rpc, _db, **kwargs):
        assert kwargs["networks"] == config.EVM_BACKFILL_ASSIST_NETWORKS
        return {
            "evm_backfill_due": 3,
            "evm_backfilled": 1,
            "evm_backfill_partial": 0,
            "evm_backfill_calls": 12,
            "evm_backfill_retry": 0,
...
async def test_cycle_stamps_last_ok_only_when_no_errors(monkeypatch):
    ...
    fresh = _DB("8453")
    await run_evm_replay.run_cycle(object(), fresh)
