"""اختبارات تصنيف الأصول — fomo منصّة متعدّدة الأصول لا سوق ميمات.

كل حالة أدناه **مشاهَدة فعلاً** في أرشيفنا (2026-07-30): أصول كبرى، عملات
مستقرّة، أسهم وسلع مرمّزة، وميمات تنتحل رموز الكبار.
"""
import config
...
# --- انتحال الرموز: أخطر حالة ---
def test_meme_impersonating_btc_is_not_major():
    """مقيس: عملة «BTC» بسعر $0.0035 وقيمة $3.5M — ميم لا بيتكوين."""
...
# --- القيمة السوقية غير المصدَّقة ---
def test_absurd_market_cap_is_ignored_not_trusted():
    """مقيس: SMILE بـ$0.0888 وقيمة سوقية «$69 تريليون» — تُتجاهل ويُحكم بالسعر."""
...
# --- الأسهم والسلع المرمّزة: القيمة السوقية لا تكشفها، السعر يكشفها ---
def test_tokenized_apple_by_price_despite_tiny_cap():
    """مقيس: AAPL بقيمة سوقية $1.36M فقط (المرمَّز جزء ضئيل) وسعر $339."""
...
def test_dollar_priced_meme_is_not_stable_if_it_moved():
    """عملة لمست الدولار لكنها تحرّكت خارج النطاق ⇒ ليست مستقرّة."""
    assert cls("HYPEDMEME", 0.4, 1.02, 5_000_000) == "meme"


# --- الحدود والقيم الناقصة ---
